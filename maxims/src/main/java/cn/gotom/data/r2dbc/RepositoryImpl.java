...
import cn.gotom.data.dao.SimpleRepositoryAbs;
...
abstract class RepositoryImpl<T, ID extends Serializable> extends WebAbsContext implements SimpleRepositoryAbs {
	...
	public RepositoryImpl(SimpleRepositoryFactory factory, RelationalEntityInformation<T, ID> entityInfo) {
		this.factory = factory;
		this.entityInfo = entityInfo;
		this.entityOperations = factory.getEntityTemplate();
	...
	public Mono<Boolean> validatorUpdate(Object update) {
		return context(ctx -> validator(ctx, update, true));
	}

	public Mono<Boolean> validatorInsert(Object insert) {
	...
	private Mono<Boolean> validatorFk(Object form, EntityProperties prop, boolean update) {
		...
		if (fk != null) {
			...
			if (fkValue == null) {
				...
				return Mono.error(CustomException.bq(prop.getComment() + "不允许为空"));
			}
...
//							+ fk.value().getSimpleName() + "[" + fkValue + "]";
					return Mono.error(CustomException.bq(prop.getComment() + "不存在"));
				} else {
					return Mono.just(true);
				}
			});
				...
				if (update) {
					if (list.size() > 0) {
						...
						if (!idValue.equals(id)) {
							return Mono.error(CustomException.bq(prop.getComment() + " [" + fieldValue + "]已经存在"));
						}
					}
				} else {
					if (list.size() > 0) {
	...
	private Mono<Integer> deleteById(ID id) {
		...
		if (log.isInfoEnabled()) {
			log.info("{}{} DELETE FROM {} WHERE {}", entityName, NEWLINE, entityTable, where);
		}
	...
	private Mono<Integer> deleteLink(Serializable fkId, LinkDelete linkDelete) {
		...
		return repository.findIds(linkDelete.column(), fkId).flatMap(link -> {
			if (log.isInfoEnabled()) {
				log.info("{}{} {} {}", getEntityName(), NEWLINE, linkDelete.value().getName(), JSON.toJSONString(link));
			}
		...
		if (log.isInfoEnabled()) {
			...
			case SELECT:
				...
				if (query.getColumns().size() > 0) {
					int i = 0;
					for (SqlIdentifier column : query.getColumns()) {
						if (i > 0) {
							sql.append(", ");
						}
						sql.append(column.getReference());
						i++;
					}
				} else {
					sql.append("* ");
